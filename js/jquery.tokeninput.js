...
(function($) {
    ...
    $.fn.tokenInput = function(url, options) {
        ...
        return this.each(function() {
            if (!$.data(this, "tokenList")) {
                $.data(this, "tokenList", new $.TokenList(this, settings));
            }
    ...
    $.TokenList = function(input, settings) {
        ...
        var input_box = $("<input type=\"text\">")
            ...
            .blur(function() {
                ...
                if (settings.defaultOptions) {
                    hide_dropdown_timeout = setTimeout(function() {
                        hide_dropdown();
                    }, 200);
                } else {
                    hide_dropdown();
            ...
            .keydown(function(e) {
                ...
                switch (kCode) {
                    ...
                    case KEY.DOWN:
                        if (!$(this).val()) {
                            ...
                            if ((previous_token.length && previous_token.get(0) === selected_token) || (next_token.length && next_token.get(0) === selected_token)) {
                                ...
                                if (kCode == KEY.LEFT || kCode == KEY.UP) {
                                    deselect_token($(selected_token), POSITION.BEFORE);
                                } else {
                                    deselect_token($(selected_token), POSITION.AFTER);
                                }
                            } else if ((kCode == KEY.LEFT || kCode == KEY.UP) && previous_token.length) {
                                // We are moving left, select the previous token if it exists
                                select_token($(previous_token.get(0)));
                            } else if ((kCode == KEY.RIGHT || kCode == KEY.DOWN) && next_token.length) {
                            ...
                            if (kCode == KEY.DOWN || kCode == KEY.RIGHT) {
                    ...
                    default:
                        if (is_printable_character(kCode)) {
                            // set a timeout just long enough to let this function finish.
                            setTimeout(function() {
                                do_search(false);
                            }, 5);
        ...
        function highlight_term(value, term) {
            return value.toString().replace(new RegExp("(?![^&;]+;)(?!<[^<>]*)(" + term + ")(?![^<>]*>)(?![^&;]+;)", "gi"), "<b>$1</b>");
        }
        ...
        function run_search(query) {
            ...
            if (cached_results) {
                ...
                if (settings.method == "POST") {
                    //console.log('post'+settings.url + queryStringDelimiter + settings.queryParam + "=" + query);
        ...
        this.add_token_api = function(id, name) {
            var pred_hidden_input = hidden_input;
            var pred_input_token = input_token;
            hidden_input = $(input);
            input_token = hidden_input.parent().find('.token-input-input-token');
            ...
            // Save this token id
            if (hidden_input.val() != null && hidden_input.val() != '')
                var id_string = "," + id;
            else
                var id_string = id;
            ...
            hidden_input = pred_hidden_input;
            input_token = pred_input_token;
        }
